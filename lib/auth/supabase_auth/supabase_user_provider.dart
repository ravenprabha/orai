...
class TaaarSupabaseUser extends BaseAuthUser {
  TaaarSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> taaarSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TaaarSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
